...
export function OnboardingCard() {
  ...
  return (
    <section
      className="mx-5 mb-8 rounded-2xl border border-[rgba(240,239,232,0.08)] bg-[#222219] py-6 px-6 md:mx-10"
    ...
    >
      ...
      <h2
        className="text-[18px] font-normal tracking-[-0.02em] text-[#F0EFE8] mb-4"
      ...
      <ul className="space-y-3">
        {data.steps.map((step) => (
          <li key={step.id} className="flex items-center gap-3">
            ...
            <span className={`text-[14px] ${step.done ? 'text-[#A09E97]' : 'text-[#F0EFE8]'}`}>
              {step.label}
            </span>
            {!step.done && links[step.id] && (
              <Link
                href={links[step.id]}
                className="ml-auto text-[13px] text-[#D9714A] hover:text-[#F0EFE8] underline underline-offset-2"
              >
                Do this →
              </Link>
            )}
