...
function parseFormData(formData: FormData) {
  ...
  return {
    ...
    garmentDescription: typeof garmentDescription === 'string' ? garmentDescription.trim() || null : null,
    swapTarget,
...
export async function POST(request: Request) {
  ...
  let user: { id: string; credits: number; suspended?: boolean } | null = await getUserIdFromApiKey(apiKey);
  if (!user) {
    const validation = await getSessionAndValidate();
    if (validation.valid) {
      const u = await prisma.user.findUnique({
        where: { id: validation.user.id },
        select: { id: true, credits: true, suspended: true },
      });
      if (u) user = { ...u, suspended: u.suspended };
    } else if (validation.deleted) {
      return NextResponse.json(
        { error: 'Account deleted', message: 'Your account has been closed. If you believe this is a mistake, contact support@virtufit.com', code: 'ACCOUNT_DELETED' },
        { status: 401 }
      );
    }
  }
  ...
  if (user.suspended) {
    return NextResponse.json(
      { error: 'Account suspended', message: 'Your VirtuFit account has been suspended. Contact support at support@virtufit.com' },
      { status: 401 }
    );
  }
  ...
  try {
    if (parsed.personImageUrl) {
      ...
      personUrl = await uploadTempImage(buffer, 'person-photos');
    } else {
    ...
    for (const f of parsed.garmentFiles) {
      ...
      const url = await uploadTempImage(buf, 'garment-photos');
  ...
  // One Replicate run per garment, sequential with delay to respect rate limit (burst 1 on low-credit accounts)
  const RATE_LIMIT_DELAY_MS = 12_000; // 12s between runs to stay under 6/min with burst 1
  ...
  if (firstError?.error || results.some((r) => r.outputUrls.length === 0)) {
    ...
    console.error('[POST /api/v1/generate] Generation failed:', details);
    return NextResponse.json(
  ...
  const jobId = jobIds[0] ?? `gen_${Date.now()}`;
